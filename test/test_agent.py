...
from src.agent import handle_transaction, initialize, ASSETS, TRANSFER_EVENTS
...
def test_handle_transaction():
    ...
    sample_transfer_event = {"block": {"timestamp": 1627651200}, "price": 1.02}
    ...
    with mock.patch(
        "src.utils.get_coin_id_by_contract_address",
        get_coin_id_by_contract_address_mock,
    ):
        ...
        with mock.patch("src.agent.Prophet", prophet_mock):
            ...
            for asset in ASSETS:
                ...
                assert (
                    f"{asset_key} depeg detected with a predicted price of 1.02"
