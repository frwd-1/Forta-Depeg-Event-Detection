...
async def my_initialize(block_event: forta_agent.block_event.BlockEvent):
    ...
    for pool in pools:
        amount = len(
            await swaps_table.get_all_rows_by_criteria(
...
async def analyze_transaction(
    ...
    for event in [
        *transaction_event.filter_log(json.dumps(swap_abi)),
        *transaction_event.filter_log(json.dumps(swap_v2_abi)),
        ...
        await swaps.paste_row(
            {
                ...
                "block": transaction_event.block_number,
        ...
        hourly_timestamp = (
            transaction_event.block.timestamp - transaction_event.block.timestamp % 60
        )
        # get these rows from the database
        future_rows = await future.get_all_rows_by_criteria(
        ...
        if future_row:
            ...
            uncertainty = (
                (future_row.price_upper - future_row.price_lower)
                if future_row
                else None
            )
            ...
            if error > 4 * uncertainty + 0.05 and critical_enable:
                ...
                name0 = get_token_name(
                    Web3.toChecksumAddress(pool.token0), erc20_abi, web3
                )
                name1 = get_token_name(
                    Web3.toChecksumAddress(pool.token1), erc20_abi, web3
                )

                findings.append(
                    SmartPriceChangesFindings.critical(
                        protocols,
                ...
                findings.append(
                    SmartPriceChangesFindings.high(
                        protocols,
                        transaction_event.to,
                        event.address,
                        future_row.price,
                ...
                findings.append(
                    SmartPriceChangesFindings.medium(
                        protocols,
                        transaction_event.to,
                        event.address,
                        future_row.price,
                ...
                price > future_row.price_upper or price < future_row.price_lower
            ) and low_enable:
                pool = await pools.get_row_by_criteria({"pool_contract": event.address})
                name0 = get_token_name(
                    Web3.toChecksumAddress(pool.token0), erc20_abi, web3
...
async def main(
    event: forta_agent.transaction_event.TransactionEvent
    | forta_agent.block_event.BlockEvent,
):
    """
...
def provide_handle_block():
    ...
    def wrapped_handle_block(block_event: forta_agent.block_event.BlockEvent) -> list:
        return [
            finding
            for findings in asyncio.run(main(block_event))
            for finding in findings
        ]
